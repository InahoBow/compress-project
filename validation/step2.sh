#!/bin/bash
HOME=`pwd`
OUT=$HOME/../pre-reassemble/out

STEP=step2
OUTZ=$HOME/$STEP/out.z
MYOUT=$HOME/$STEP/out

MYDEFLATE=$HOME/../mydeflate/mydeflate
MYSHRINK=$HOME/../shrink/shrnk
...
rm -f $HOME/$STEP/*sta*csv $HOME/$STEP/*/*
...
stati()
{
	...
	echo "$count:   raw $raw_default_len, rawsmall $raw_small_len, db $db_default_len, dbsmall $db_small_len, dbs $db_s_len"
...
echo ",raw,raw2,diff_bit_d,diff_bit_d2,diff_bit_s" > $filenamez
